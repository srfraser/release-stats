...
for i, workertype in enumerate(df['workertype'].unique()):
    ...
    if workertype in ['human-decision', 'buildbot-bridge', 'buildbot']:
        continue
    some_data.append({
        'y': results,
        'type': 'box',
...
plotly.offline.plot(go.Figure(data=some_data, layout=layout), filename='task-delays-filtered-more.html')
